...
impl Template {
    pub fn bindings(
        ...
        let benchmark_out_path = out_path
            .join("../benchmarks/src")
            .join(self.out_subdir)
            .join(algorithm);
        let trait_out_path = out_path
            .join("../traits/src")
            .join(self.out_subdir)
            .join(algorithm);
        ...
        std::fs::create_dir_all(&benchmark_out_path)?;
        std::fs::create_dir_all(&trait_out_path)?;
        let mut modules = vec![];
        for descriptor in descriptors {
            ...
            std::fs::write(
                out_path.join(format!("{profile}.rs")),
                params.apply(self.lib_template).as_bytes(),
            )?;
            std::fs::write(
                benchmark_out_path.join(format!("{profile}.rs")),
                params.apply(self.benchmark_template).as_bytes(),
            )?;
            std::fs::write(
                trait_out_path.join(format!("{profile}.rs")),
        ...
        std::fs::write(out_path.join("mod.rs"), module)?;
        std::fs::write(benchmark_out_path.join("mod.rs"), module)?;
        std::fs::write(trait_out_path.join("mod.rs"), module)?;
        Ok(())
    }
}
