...
const VTABLE: Vtable<SivMac> = Vtable::<SivMac> {
    ...
    init: haberdashery_sivmac_broadwell_init,
    sign: haberdashery_sivmac_broadwell_sign,
    verify: haberdashery_sivmac_broadwell_verify,
};
...
fn init(iters: u64, measure: &mut dyn Measure) {
    if unsafe { 0 == haberdashery_sivmac_broadwell_is_supported() } {
        measure.skip();
        return;
    }
