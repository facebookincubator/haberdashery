...
use haberdashery_sys::mac::sivmac::haswell::*;
...
const VTABLE: Vtable<SivMac> = Vtable::<SivMac> {
    ...
    init: haberdashery_sivmac_haswell_init,
    sign: haberdashery_sivmac_haswell_sign,
    verify: haberdashery_sivmac_haswell_verify,
};
...
fn init(iters: u64, measure: &mut dyn Measure) {
    if unsafe { 0 == haberdashery_sivmac_haswell_is_supported() } {
        measure.skip();
        return;
    }
