...
use haberdashery_sys::mac::sivmac::skylakex::*;
...
const VTABLE: Vtable<SivMac> = Vtable::<SivMac> {
    ...
    init: haberdashery_sivmac_skylakex_init,
    sign: haberdashery_sivmac_skylakex_sign,
    verify: haberdashery_sivmac_skylakex_verify,
};
...
fn init(iters: u64, measure: &mut dyn Measure) {
    if unsafe { 0 == haberdashery_sivmac_skylakex_is_supported() } {
        measure.skip();
        return;
    }
