...
const VTABLE: Vtable<Aes256Gcm> = Vtable::<Aes256Gcm> {
    ...
    init: haberdashery_aes256gcm_skylake_init,
    encrypt: haberdashery_aes256gcm_skylake_encrypt,
    decrypt: haberdashery_aes256gcm_skylake_decrypt,
};
...
fn init(iters: u64, measure: &mut dyn Measure) {
    if unsafe { 0 == haberdashery_aes256gcm_skylake_is_supported() } {
        measure.skip();
        return;
    }
