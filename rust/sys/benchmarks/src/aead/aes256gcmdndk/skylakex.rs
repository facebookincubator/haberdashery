...
const VTABLE: Vtable<Aes256GcmDndk> = Vtable::<Aes256GcmDndk> {
    ...
    init: haberdashery_aes256gcmdndk_skylakex_init,
    encrypt: haberdashery_aes256gcmdndk_skylakex_encrypt,
    decrypt: haberdashery_aes256gcmdndk_skylakex_decrypt,
};
...
fn init(iters: u64, measure: &mut dyn Measure) {
    if unsafe { 0 == haberdashery_aes256gcmdndk_skylakex_is_supported() } {
        measure.skip();
        return;
    }
