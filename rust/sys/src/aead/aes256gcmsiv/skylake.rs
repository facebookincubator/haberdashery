...
extern "C" {
    pub fn haberdashery_aes256gcmsiv_skylake_init(this: *mut Aes256GcmSiv, key: *const u8, key_len: usize) -> i32;
...
fn test() {
    unsafe {
        if 0 == haberdashery_aes256gcmsiv_skylake_is_supported() {
            return;
        }
        ...
        assert_ne!(haberdashery_aes256gcmsiv_skylake_init(this.as_mut_ptr(), key.as_ptr(), key.len()), 0);
        assert_ne!(haberdashery_aes256gcmsiv_skylake_encrypt(
            this.as_ptr(),
            nonce.as_ptr(),
            nonce.len(),
            aad.as_ptr(),
        ...
        assert_ne!(haberdashery_aes256gcmsiv_skylake_decrypt(
            this.as_ptr(),
            nonce.as_ptr(),
            nonce.len(),
            aad.as_ptr(),
