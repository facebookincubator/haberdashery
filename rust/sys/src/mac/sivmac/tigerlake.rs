...
extern "C" {
    pub fn haberdashery_sivmac_tigerlake_init(this: *mut SivMac, key: *const u8, key_len: usize) -> i32;
...
fn test() {
    unsafe {
        ...
        assert_ne!(haberdashery_sivmac_tigerlake_init(this.as_mut_ptr(), key.as_ptr(), key.len()), 0);
        assert_ne!(haberdashery_sivmac_tigerlake_sign(
            this.as_ptr(),
            message.as_ptr(),
            message.len(),
            tag.as_mut_ptr(),
        ...
        assert_ne!(haberdashery_sivmac_tigerlake_verify(
            this.as_ptr(),
            message.as_ptr(),
            message.len(),
            tag.as_ptr(),
