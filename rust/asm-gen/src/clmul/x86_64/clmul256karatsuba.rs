...
pub struct ClMul256KaratsubaState<const N: usize> {
    lhs: [Block256; N],
    rhs: [Block256; N],
    pub product: ClMul256KaratsubaProduct,
    pub index: usize,
}
impl<const N: usize> ClMul256KaratsubaState<N> {
    #[inline]
    pub fn new(state: Block256, mut lhs: [Block256; N], rhs: [Block256; N]) -> Self {
        lhs[0] ^= state;
        Self {
            lhs,
            rhs,
            product: ClMul256KaratsubaProduct::default(),
            index: 0,
        }
    }
    #[inline]
    pub fn round(&mut self) {
        ...
        match self.index {
            0 => self.product = self.lhs[0].clmul256karatsuba(self.rhs[N - 1]),
            _ => {
                self.product ^= self.lhs[self.index].clmul256karatsuba(self.rhs[N - 1 - self.index])
            }
        }
        self.index += 1;
    }
    #[inline]
    pub fn state(&self) -> Block256 {
        debug_assert_eq!(self.index, N);
        self.product.reduce()
    }
    #[inline]
    pub fn advance(&mut self) -> (Block256, Block256) {
        debug_assert!(self.index < N, "{index} >= {N}", index = self.index);
