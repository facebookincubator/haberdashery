...
impl ClMul256Karatsuba for M256i {
    type Input = M256i;
    #[inline]
    fn clmul256karatsuba(self, rhs: Self::Input) -> ClMul256KaratsubaProduct {
...
impl ClMul256Karatsuba for M128i {
    type Input = M256i;
    #[inline]
    fn clmul256karatsuba(self, rhs: Self::Input) -> ClMul256KaratsubaProduct {
...
impl<const N: usize> ClMul256Karatsuba for [M128i; N] {
    type Input = [M256i; N];
    #[inline]
    fn clmul256karatsuba(self, rhs: Self::Input) -> ClMul256KaratsubaProduct {
...
impl<const N: usize> ClMul256KaratsubaPowerTable<N> {
    ...
    #[inline]
    pub fn key(self) -> M128i {
        self.key
    }
    #[inline]
    pub fn clmul256karatsuba(self, array: [M256i; N]) -> ClMul256KaratsubaProduct {
        self.powers.clmul256karatsuba(array)
    }
    #[inline]
    pub fn clmul256karatsuba_reader(self, mut state: M256i, mut reader: Reader) -> M256i {
            ...
            assert_eq!(
                {
                    ...
                    table.clmul256karatsuba_reader(M256i::ZERO, reader)
                },
                {
            ...
            assert_eq!(
                {
                    let table = ClMul256KaratsubaPowerTable::<1>::new(key);
                    let reader = Reader::from(data.as_slice());
                    let state = table.clmul256karatsuba_reader(M256i::ZERO, reader);
