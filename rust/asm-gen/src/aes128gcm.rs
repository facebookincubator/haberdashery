...
impl<const N: usize> From<M128i> for Aes128GcmKey<N> {
    #[inline]
    fn from(key: M128i) -> Self {
        let aes = Aes128::new(key);
        let polyval = ClMul128FoilPowerTable::new_ghash(aes.encrypt(M128i::zero()));
        Self { aes, polyval }
    }
}
...
impl<const N: usize> Aes128GcmState<N> {
    ...
    #[inline]
    pub fn finalize(&mut self, key: &Aes128GcmKey<N>, is_encrypt: bool) -> M128i {
        if self.crypt_len == 0 {
            self.finalize_aad(key);
        }
        if self.crypt_len > 0 {
