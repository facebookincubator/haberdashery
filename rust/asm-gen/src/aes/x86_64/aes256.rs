...
impl<T: AesRoundInput + Copy> Aes256EncryptState<T> {
    ...
    pub fn aes_round(&mut self) {
        match self.index {
            0 => self.aes.encrypt_round_first(&mut self.data),
            1..14 => self.aes.encrypt_round(&mut self.data, self.index),
...
impl Aes256 {
    ...
    #[inline]
    pub fn new_aesenclast(key: [Block128; 2]) -> Self {
    ...
    #[inline]
    pub fn new_aeskeygenassist(key: [Block128; 2]) -> Self {
        let mut this = Self::zero();
        this.0[0] = key[0];
        this.0[1] = key[1];
    ...
    #[inline]
    pub fn expand_step_aesenclast<const N: usize>(
        &mut self,
...
mod tests {
    ...
    fn new_and_encrypt() {
        fn test_new_and_encrypt<const N: usize>() {
            for _ in 0..128 {
                let key: [Block128; 2] = random::random();
                let plaintext: [Block128; N] = random::random();
