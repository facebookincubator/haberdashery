...
typedef struct haberdashery_aes256gcm_streaming_tigerlake_key_t {
  uint8_t value[336];
} haberdashery_aes256gcm_streaming_tigerlake_key_t;

__attribute__((aligned(16)))
typedef struct haberdashery_aes256gcm_streaming_tigerlake_state_t {
  uint8_t value[112];
} haberdashery_aes256gcm_streaming_tigerlake_state_t;

int haberdashery_aes256gcm_streaming_tigerlake_init_key(
    haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    const unsigned char *key_bytes,
    size_t key_bytes_len);
int haberdashery_aes256gcm_streaming_tigerlake_init_state(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    const unsigned char *nonce,
    size_t nonce_len);
int haberdashery_aes256gcm_streaming_tigerlake_aad_update(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    const unsigned char *aad,
    size_t aad_len);
int haberdashery_aes256gcm_streaming_tigerlake_encrypt_update(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
...
int haberdashery_aes256gcm_streaming_tigerlake_encrypt_finalize(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    unsigned char *tag,
    size_t tag_len);
int haberdashery_aes256gcm_streaming_tigerlake_decrypt_update(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    const unsigned char *plaintext,
    size_t plaintext_len,
    unsigned char *ciphertext,
    size_t ciphertext_len);
int haberdashery_aes256gcm_streaming_tigerlake_decrypt_finalize(
    haberdashery_aes256gcm_streaming_tigerlake_state_t *state,
    const haberdashery_aes256gcm_streaming_tigerlake_key_t *key,
    const unsigned char *tag,
    size_t tag_len);
int haberdashery_aes256gcm_streaming_tigerlake_is_supported(void);
...
static int do_cipher_fn(
  ...
  if (in == NULL) { /* doFinalize called */
    ...
    return haberdashery_aes256gcm_streaming_tigerlake_aad_update(
        &(ctx_data->state), &(ctx_data->key), in, inl);
  } else { /* doCrypt for ciphertext */
    if (1 == EVP_CIPHER_CTX_encrypting(ctx)) { /* encrypt */
      return haberdashery_aes256gcm_streaming_tigerlake_encrypt_update(
          &(ctx_data->state), &(ctx_data->key), in, inl, out, inl);
    } else { /* decrypt */
      return haberdashery_aes256gcm_streaming_tigerlake_decrypt_update(
          &(ctx_data->state), &(ctx_data->key), in, inl, out, inl);
    }
  }
  return 0;
...
const EVP_CIPHER *haberdashery_aes256gcm_tigerlake_EVP_CIPHER(void) {
  EVP_CIPHER *cipher;
  if (0 == haberdashery_aes256gcm_streaming_tigerlake_is_supported()) {
    return NULL;
  }
