...
static int do_cipher_fn(
  ...
  if (in == NULL) { /* doFinalize called */
    ...
    return haberdashery_aes128gcm_streaming_skylakex_aad_update(
        &(ctx_data->state), &(ctx_data->key), in, inl);
  } else { /* doCrypt for ciphertext */
    if (1 == EVP_CIPHER_CTX_encrypting(ctx)) { /* encrypt */
      return haberdashery_aes128gcm_streaming_skylakex_encrypt_update(
          &(ctx_data->state), &(ctx_data->key), in, inl, out, inl);
    } else { /* decrypt */
      return haberdashery_aes128gcm_streaming_skylakex_decrypt_update(
          &(ctx_data->state), &(ctx_data->key), in, inl, out, inl);
    }
  }
  return 0;
...
const EVP_CIPHER *haberdashery_aes128gcm_skylakex_EVP_CIPHER(void) {
  EVP_CIPHER *cipher;
  if (0 == haberdashery_aes128gcm_streaming_skylakex_is_supported()) {
    return NULL;
  }
