...
int main(void) {
  int res;
  const EVP_CIPHER *evp_cipher = haberdashery_aes128gcm_broadwell_EVP_CIPHER();
  ...
  res = encrypt_decrypt(EVP_aes_128_gcm(), evp_cipher);
  if (0 != res) {
    return 16 + res;
  }
  res = encrypt_decrypt(evp_cipher, EVP_aes_128_gcm());
  if (0 != res) {
    return 32 + res;
  }
  return 0;
