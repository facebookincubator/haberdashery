...
use haberdashery_sys::aead::aes256gcmdndk_neoversev2::*;
pub use haberdashery_sys::aead::aes256gcmdndk_neoversev2::Aes256Gcmdndk;
...
impl Aead for Aes256Gcmdndk {
    ...
    const FN_ENCRYPT: unsafe extern "C" fn(
        ...
        tag: *mut u8,
        tag_len: usize,
    ) -> i32 = haberdashery_aes256gcmdndk_neoversev2_encrypt;
    const FN_DECRYPT: unsafe extern "C" fn(
        ...
        plaintext: *mut u8,
        plaintext_len: usize,
    ) -> i32 = haberdashery_aes256gcmdndk_neoversev2_decrypt;
...
mod benchmarks {
    ...
    fn init(iters: u64, measure: &mut dyn perf_caliper::Measure) {
        let Some(mut context) = crate::benchmark::aead::Context::<super::Aes256Gcmdndk>::aad(0) else {
            measure.skip();
            return;
        };
        measure.start();
    ...
    fn aad(length: usize, iters: u64, measure: &mut dyn perf_caliper::Measure) {
        let Some(mut context) = crate::benchmark::aead::Context::<super::Aes256Gcmdndk>::aad(length) else {
            measure.skip();
            return;
        };
        measure.start();
    ...
    fn encrypt(length: usize, iters: u64, measure: &mut dyn perf_caliper::Measure) {
        let Some(mut context) = crate::benchmark::aead::Context::<super::Aes256Gcmdndk>::crypt(length) else {
            measure.skip();
            return;
        };
        measure.start();
...
mod tests {
    #[test]
    fn cozybuf() {
        let result = crate::vectors::aead::verify::<super::Aes256Gcmdndk>("aes256gcmdndk.cozybuf");
        if cfg!(feature = "gen") && result.is_err() {
            crate::vectors::aead::generate::<super::Aes256Gcmdndk>("aes256gcmdndk.cozybuf").unwrap();
        } else {
            result.unwrap();
        }
    }
